...
- Fixed HA0001 so split argument and parameter lists expand each item onto its own line, and expression-bodied members with hanging parameter lists report and fix stranded `=>` tokens by keeping `) =>` on one line.
